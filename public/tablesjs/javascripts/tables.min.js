...
var tablesJs = {
	...
	add: function(parent) {
		...
		// get new row number for pass to event
		rownum = tablesJs.count(parent);

		// iterate the hedaer columns (th)
		var ths = thead.getElementsByTagName('th');
		for (i=0; i<ths.length; i++) {
			th = ths[i];
			// get new cold number for pass to event
			colnum = i;
			// create cell (td)
			...
			if (tablesJs.events[parent.id+".on_edit_cell"]) {
				// call the function
				input.onkeyup = tablesJs.events[parent.id+".on_edit_cell"](rownum, colnum);
			}
	 ...
	 * 
	 */
	get_color: function(parent, rownum, colnum. x) {
		var input = tablesJs.get_input(parent, rownum, colnum);
